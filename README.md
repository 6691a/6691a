...
SI에서 개발되어 관리되지 않고 명세서 없는 서비스를 이어 받아 문서화, 코드 정리를 하며 1인 백엔드 개발자로 
간단한 프로젝트 일정 관리, 버그 및 이슈 브리핑 및 상황 대처를 하며 
기능 구축부터 인프라 비용 최적화, 레거시 리팩토링, 결제·정산 자동화까지 스타트업이 필요로 하는 전 영역을 직접 수행해 왔습니다.

인프라 비용 절감, 배치 작업 단축, 일정관리 등을 관리하며, 소수 인원으로 빠르게 움직여야 하는 환경에 익숙합니다.
