...
결제·정산·미디어 파이프라인등 여러 도메인을 거부감 없이 학습하고 다뤄온 Python 백엔드 개발자입니다.

관리되지 않던 레거시 서비스와 인프라를 인수해 구조를 파악하고, 문서화와 운영 안정화부터 신규 기능 개발까지 수행해 왔습니다. 인프라 비용 약 40% 절감, 정산 배치 30분  10분, 매출 API 7분  3분 등 비용과 성능을 함께 고려한 개선 경험이 있습니다.

현재는 FastAPI · LangChain · LangGraph · PostgreSQL(pgvector) 기반 시장 분석 서비스를 직접 설계·구현하며 RAG 검색, LLM 파이프라인, 데이터 수집·배치 구조를 학습하고 있습니다.

기능을 구현하기 전에 문제와 목적을 먼저 정의하고, 기술 선택과 의사결정의 근거를 문서로 남기는 것을 중요하게 생각합니다.
