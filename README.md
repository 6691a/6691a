...
1인 백엔드 전담으로 일정·이슈 관리부터 인프라, 기능 개발까지 백엔드 전반을 책임져 온 Python 개발자입니다. 결제·정산 자동화, Celery 비동기 처리, 인프라 비용 최적화, AWS Lambda·MediaConvert 기반 서버리스 미디어 처리(WebP/HLS) 파이프라인을 직접 구축했고, 비용·성능·운영 효율을 함께 고려해 작은 팀에서 빠르게 임팩트를 만드는 데 강점이 있습니다.
