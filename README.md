...
5년차 백엔드 개발자입니다. 0→1 기능 구축부터 인프라 비용 최적화, 레거시 리팩토링, 결제·정산 자동화까지 스타트업이 필요로 하는 전 영역을 직접 수행해 왔습니다.
...
- **풀스택 지향** — 백엔드를 메인으로 하되, 필요하면 인프라·어드민·크롤러·결제 연동까지 직접 수행합니다.
...
- 정기적으로 금감원에 제출해야 하는 정산 관련 내역을 어드민에서 자동 다운로드 받을 수 있도록 구현 — 반복 컴플라이언스 업무 자동화
