...
관리되지 않던 레거시 서비스와 빈약한 문서를 인수해 문서화, 코드 정리, 운영 안정화, 신규 기능 개발까지 수행해 온 Python 백엔드 개발자입니다.

1인 백엔드 전담 환경에서 결제·정산 자동화, Celery 기반 비동기 처리, 인프라 비용 최적화, 미디어 처리(WebP/HLS), API 문서화 등을 직접 담당했습니다.

특히 작은 팀에서 빠르게 문제를 정의하고, 비용·성능·운영 효율을 함께 고려해 실질적인 비즈니스 임팩트를 만드는 데 강점이 있습니다.
...
- **레거시를 두려워하지 않음** — 오래되고 관리되지 않은 코드라도 버그, 꼭 개선이 필요한 상황이라면 주저하지 않습니다.
...
- SI 외주 시절 생성된 필요 이상의 인프라, 사용 의도를 알 수 없는 인프라 식별 후 제거 및 통합 → **인프라 비용 약 40% 절감**
